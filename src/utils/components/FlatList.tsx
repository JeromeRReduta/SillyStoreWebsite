...
import css from "../css/flat-list.module.css";

/** Lovingly stolen from react native - credit to them */
export default function FlatList<TData>({
    listClassName = css.flat_list,
    listItemClassName = css.flat_list_item,
    data,
    renderItem,
    keyExtractor,
}: {
    ...
    return (
        <section className={listClassName}>
            {data.map((elem) => (
                <div key={keyExtractor(elem)} className={listItemClassName}>
                    {renderItem(elem)}
                </div>
            ))}
        </section>
